...
def draw_tree(tree_root):
    ...
    nx.draw(tree, pos=pos, labels=labels, arrows=False,
            node_size=2500, node_color=colors, font_color="red")
    plt.show()


def color_get(cs):
    r, g, b = [int(255 - (255 / (cs+1))) for _ in range(3)]
...
def dfs(node, cs=0):
    if node:
        ...
        node.color = color_get(cs)
        if node.left:
            cs = dfs(node.left, cs + 2)
        if node.right:
            cs = dfs(node.right, cs + 2)
